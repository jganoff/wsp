...
- **Review your own diff before opening the PR.** Read `git diff <base>..HEAD` end to end as a critic, fix everything, simplify, and only then open it — opening a PR means it is ready for another person. A PR opened early gets merged early, and any finding the review produces afterwards lands nowhere and needs a second PR against a moved `main`. Things worth hunting for: duplicated calls, comparisons that should be direct checks, comment claims that are not true on every platform, tables and constants with no test, and anything deletable.
...
- **New user-facing command or flag: add a smoke check.** `scripts/smoke.sh` and its PowerShell twin `scripts/smoke.ps1` are the only tests that run a real binary against a real filesystem on macOS, Linux, and Windows. Unit tests share the binary's assumptions and miss whole classes of bug: `wsp ls` printing "No workspaces." while swallowing the footer that said a workspace was recoverable passed every unit test and failed the smoke script on its first run. Prefer a real test where one is possible — reach for the smoke scripts for the end-to-end path, the cross-platform behaviour, and anything the unit tests structurally cannot see. Add the check to **both** scripts, and **put it in the offline half if it can possibly go there** — that half runs on every PR from `ci.yml`, while the network half still waits for a release dispatch, where a failure blocks a tag. Offline is further than it looks: `wsp new --empty` plus an unregistered URL covers most of the surface without a clone. A check must assert observable state, not an exit code; `wsp foo --help` exiting 0 is worth nothing. `test_every_command_is_smoked_or_listed_as_a_gap` in `cli/completion.rs` enforces command-level coverage against an `UNSMOKED` debt register — the assertion is an equality, so it fails both when a command loses coverage and when the register goes stale. Flag-level coverage is convention, not enforced.
