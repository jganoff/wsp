...
- **`git.*` config keys are validated against a denylist**: `wsp config set git.*` and `wsp template config set git.*` reject keys that can execute arbitrary commands (e.g. `core.sshCommand`, `core.hooksPath`, `filter.*`). The **canonical** denylist lives in `crates/wsp-core/src/config.rs::DANGEROUS_GIT_CONFIG_KEY_PREFIXES` and the validator is `config::validate_git_config_key()`. Any new code path that writes git config from user input **must** call this validator before writing. `workspace::is_dangerous_git_config_key()` and `template::apply_config()` also call this same validator — **do not add a separate denylist**. Template import (`load_from_file`) also validates on load so malicious shared templates can't bypass the interactive check.
