...
7. **Don't duplicate unix.** If something is easy to do by piping `--json` output through `jq`, `grep`, or other standard tools, don't add a flag for it. Compose, don't accumulate.
   `wsp ls --quiet` is a narrow exception: it emits workspace names only for direct use as
   positional inputs to `wsp` batch commands. Use `wsp rm -- $(wsp ls -q)` and put flags before
   `--`; agents and other programmatic consumers use `--json`.
